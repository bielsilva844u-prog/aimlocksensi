...
export default function AuxilioAndroidPage() {
  ...
  useEffect(() => {
    ...
    if (!user.activatedProducts?.includes('AUXILIO-ANDROID')) {
      toast({
        ...
        description: 'Voce nao tem acesso ao AUXILIO DE MIRA ANDROID. Ative uma chave para continuar.',
      });
      router.push('/inicio');
    }
  ...
  if (loading || !user || !user.activatedProducts?.includes('AUXILIO-ANDROID')) {
